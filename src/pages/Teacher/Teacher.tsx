...
import { modalInterface, paginationInterface, teacherInterface, } from '@/common/allInterface';
...
const Teacher = () => {
  ...
  const getAllteacher = async (page: number = 1, limit: number = 10): Promise<void> => {
    setloading(true);
    try {
      const response = await authAxios().get(`/teacher`, {
        params: {
  ...
  const handleDelete = async (): Promise<void> => {
    try {
      const response = await authAxios().put(`/teacher/${modalState?.data?._id}`, { isDeleted: !modalState?.data?.isDeleted, });
  ...
  const handlePageChange = (newPage: number) => {
    getAllteacher(newPage, pagination.limit);
  };
        ...
        <div className='mb-2 flex flex-wrap items-center justify-between space-y-2 gap-x-4'>
          <div>
            ...
            <p className='text-muted-foreground'>
              Here&apos;s a list of your all  teacher
            </p>
          </div>
        </div>
        <div className="mb-6">
